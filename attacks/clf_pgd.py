...
def clf_pgd(x, y, diffusion, network_clf, config,model = None):
    ...
    if config.attack.if_targeted:
        print(y)
        y = _create_random_target(y,config.device)
        print(y)
        y = TargetedMisclassification(y)
    fmodel = foolbox.PyTorchModel(network_clf, device=config.device, bounds=(0., 1.), preprocessing=foolbox_preprocess(config.structure.dataset))
